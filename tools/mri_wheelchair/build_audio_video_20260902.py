...
ASSET_DIR = ROOT / "reel_assets" / "mri_series" / "mri_wheelchair_20260902"
FRAME_DIR = ROOT / "reel_assets" / "mri_series" / "mri_wheelchair_20260902_telop_frames"
AUDIO_DIR = ASSET_DIR / "audio"
WORK_DIR = ASSET_DIR / "_video_work"
VIDEO_DIR = ASSET_DIR / "video"
FINAL_DIR = ROOT / "01_ショート動画_リール_YouTubeShorts" / "インスタ完成形"
QA_DIR = ASSET_DIR / "qa_midframes"
MANIFEST = ASSET_DIR / "video_manifest_20260902.json"
...
TITLE = "車椅子だけどMRI受けられるのかな_20260902"
OUT = VIDEO_DIR / f"{TITLE}.mp4"
FINAL_OUT = FINAL_DIR / OUT.name

FRAMES = [
    "telop_01_opening_wheelchair_home.png",
    "telop_02_reception_consultation.png",
    "telop_03_wheelchair_outside_mri_room.png",
    "telop_04_prepare_single_table.png",
    "telop_05_transfer_support_single_table.png",
    "telop_06_on_table_ready_for_mri.png",
    "telop_07_pre_exam_consult_support_needs.png",
    "telop_08_return_to_wheelchair_after_exam.png",
    "telop_09_appointment_note_wheelchair.png",
    "telop_10_save_cta_phone.png",
]

DISPLAY_NARRATION = [
    "車椅子だけど、MRIって受けられるのかな。",
    "車椅子を使っている方も、MRI検査は受けられます。",
    "ただし、車椅子をMRI室に入れることはできません。",
    "施設によっては、MRI室の外で寝台へ移る方法をとることもあります。",
    "移るときは、職員が付き添ってサポートします。",
    "寝台に移ったあとは、そのまま検査へ進みます。",
    "立ち上がりに不安がある、支えが必要、痛い場所がある。そういうことは予約時に伝えておくと安心です。",
    "検査が終わったあとも、同じように寝台から車椅子へ戻ります。",
    "車椅子を使っていることだけでなく、移るときに心配なことも一緒に伝えてください。",
    "MRIの予定がある方やご家族は、検査前日に見返せるよう保存してください。次回は、杖を使っている方のMRIについてお話しします。",
]

VOICE_TEXT = [
    "車椅子だけど、エムアールアイって受けられるのかな。",
    "車椅子を使っているかたも、エムアールアイ検査は受けられます。",
    "ただし、車椅子をエムアールアイ室に入れることはできません。",
    "施設によっては、エムアールアイ室の外で寝台へ移る方法をとることもあります。",
    "移るときは、職員が付き添ってサポートします。",
    "寝台に移ったあとは、そのまま検査へ進みます。",
    "立ち上がりに不安がある、支えが必要、痛い場所がある。そういうことは予約時に伝えておくと安心です。",
    "検査が終わったあとも、同じように寝台から車椅子へ戻ります。",
    "車椅子を使っていることだけでなく、移るときに心配なことも一緒に伝えてください。",
    "エムアールアイの予定があるかたやご家族は、検査前日に見返せるよう保存してください。次回は、杖を使っているかたのエムアールアイについてお話しします。",
]


def run(cmd: list[Path | str], capture: bool = False, check: bool = True) -> subprocess.CompletedProcess[str] | None:
...
def make_qa_contact_sheet(paths: list[Path], output: Path) -> None:
    ...
    for i, path in enumerate(paths):
        ...
        draw.text((x + 10, y + thumb_h + 8), f"{i + 1:02d}", font=label_font, fill=(12, 34, 64))
...
def main() -> None:
    ...
    run(
        [
            ...
            "[voice][bgm]amix=inputs=2:duration=first:dropout_transition=0.2:normalize=0,"
