from pathlib import Path
import json
import shutil
import subprocess
import urllib.parse
import urllib.request
import wave


ROOT = Path(r"F:\ANRYCAMPANY")
ASSET_DIR = ROOT / "reel_assets" / "ct_series" / "ct_duration_v1"
FRAME_DIR = ASSET_DIR / "telop_frames"
AUDIO_DIR = ASSET_DIR / "audio"
WORK_DIR = ASSET_DIR / "_video_work"
MANIFEST = ASSET_DIR / "video_manifest.json"

FFMPEG = ROOT / "tools" / "ffmpeg" / "bin" / "ffmpeg.exe"
BGM = (
    ROOT
    / "01_ショート動画_リール_YouTubeShorts"
    / "インスタリール用"
    / "BGM フリー素材"
    / "Kind_Heart.mp3"
)
OUT = ASSET_DIR / "CTって何分くらいかかるの.mp4"
FINAL_OUT = (
    ROOT
    / "01_ショート動画_リール_YouTubeShorts"
    / "インスタ完成形"
    / "CTって何分くらいかかるの.mp4"
)

VOICEVOX = "http://127.0.0.1:50021"
SPEAKER = 20
VOICE_SPEED = 1.2
BGM_VOLUME = "-25dB"

NARRATION = [
    "CTって、どのくらい時間がかかるんだろう。予定が気になって、不安になっていませんか。",
    "検査時間がわからないと、仕事や家族のことが頭をよぎりますよね。事前に知っておくだけで、気持ちがずいぶん楽になります。",
    "CTの撮影自体は、一回あたり数秒から一分程度で終わることが多いです。ただし、準備や着替えを含めると、もう少し時間がかかります。",
    "造影剤を使わない単純CTは、検査室に入ってから終了まで、おおよそ五から十分程度が目安です。撮影そのものは、あっという間に終わります。",
    "造影剤を使うCTは、単純CTより時間がかかります。点滴の準備や、注射後に撮影タイミングを合わせる待機があるためです。",
    "造影CTは、検査室での準備から終了まで、三十分前後が目安です。ただし、検査の目的や施設の流れによって、さらに時間がかかることがあります。",
    "腎臓や尿管を詳しく見る検査では、造影剤を注射したあと、しばらく時間をおいてから追加の撮影を行うことがあります。そのため、他のCTよりも待ち時間が長くなることがあります。",
    "造影CTでは、事前に血液検査が必要な場合があります。検査結果が出てから撮影を行うため、その待ち時間が加わることもあります。",
    "受付から会計までの時間は、病院の混み具合によっても変わります。検査そのものの時間と、待ち時間は別物と考えておくと安心です。",
    "検査にかかる時間が気になる場合は、受付や担当スタッフに事前に聞いてみてください。教えてもらえますよ。",
    "検査時間を知っておくだけで、当日の安心感がぐっと変わります。だいたいこのくらい、と頭に入れて、落ち着いて検査に臨んでください。",
    "この投稿が参考になったら保存を。ほかにも検査の不安を解消する投稿をしています。フォローして、次の投稿も受け取ってください。",
]

FRAMES = [
    "slide01_telop.png",
    "slide02_telop.png",
    "slide03_telop.png",
    "slide04_telop.png",
    "slide05_telop.png",
    "slide06_telop.png",
    "slide07_telop.png",
    "slide08_telop.png",
    "slide09_telop.png",
    "slide10_telop.png",
    "slide11_telop.png",
    "slide12_telop.png",
]


def run(cmd):
    subprocess.run([str(part) for part in cmd], check=True)


def post_json(path, params=None, payload=None):
    query = urllib.parse.urlencode(params or {})
    url = f"{VOICEVOX}{path}"
    if query:
        url += f"?{query}"
    body = None if payload is None else json.dumps(payload, ensure_ascii=False).encode("utf-8")
    req = urllib.request.Request(url, data=body, method="POST")
    if body is not None:
        req.add_header("Content-Type", "application/json")
    with urllib.request.urlopen(req, timeout=60) as res:
        return res.read()


def synthesize_voice(text: str, out: Path):
    query = json.loads(post_json("/audio_query", {"text": text, "speaker": SPEAKER}))
    query["speedScale"] = VOICE_SPEED
    query["pitchScale"] = 0.0
    query["intonationScale"] = 0.96
    query["volumeScale"] = 1.0
    query["prePhonemeLength"] = 0.08
    query["postPhonemeLength"] = 0.18
    out.write_bytes(post_json("/synthesis", {"speaker": SPEAKER}, query))


def wav_duration(path: Path) -> float:
    with wave.open(str(path), "rb") as wav:
        return wav.getnframes() / wav.getframerate()


def min_duration_for(index: int) -> float:
    if index in {3, 4, 6, 7, 9, 11, 12}:
        return 6.0
    if index in {2, 5, 8, 10}:
        return 5.2
    return 4.4


def main():
    AUDIO_DIR.mkdir(parents=True, exist_ok=True)
    WORK_DIR.mkdir(parents=True, exist_ok=True)
    FINAL_OUT.parent.mkdir(parents=True, exist_ok=True)

    frames = [FRAME_DIR / name for name in FRAMES]
    for required in [FFMPEG, BGM, *frames]:
        if not required.exists():
            raise FileNotFoundError(required)

    padded_wavs = []
    durations = []
    for idx, text in enumerate(NARRATION, start=1):
        voice = AUDIO_DIR / f"voice_{idx:02d}.wav"
        padded = WORK_DIR / f"voice_{idx:02d}_padded.wav"
        synthesize_voice(text, voice)
        duration = max(min_duration_for(idx), wav_duration(voice) + 0.45)
        durations.append(duration)
        run(
            [
                FFMPEG,
                "-y",
                "-i",
                voice,
                "-af",
                f"apad=pad_dur={duration:.3f},atrim=duration={duration:.3f}",
                "-ar",
                "44100",
                "-ac",
                "2",
                padded,
            ]
        )
        padded_wavs.append(padded)

    frames_txt = WORK_DIR / "frames.txt"
    with frames_txt.open("w", encoding="utf-8") as f:
        for frame, duration in zip(frames, durations):
            f.write(f"file '{frame.as_posix()}'\n")
            f.write(f"duration {duration:.3f}\n")
        f.write(f"file '{frames[-1].as_posix()}'\n")

    voice_txt = WORK_DIR / "voice_segments.txt"
    with voice_txt.open("w", encoding="utf-8") as f:
        for wav in padded_wavs:
            f.write(f"file '{wav.as_posix()}'\n")

    silent_video = WORK_DIR / "silent.mp4"
    voice_all = AUDIO_DIR / "voice.wav"
    run(
        [
            FFMPEG,
            "-y",
            "-f",
            "concat",
            "-safe",
            "0",
            "-i",
            frames_txt,
            "-vf",
            "scale=1080:1920,format=yuv420p",
            "-r",
            "30",
            "-c:v",
            "libx264",
            "-pix_fmt",
            "yuv420p",
            silent_video,
        ]
    )
    run([FFMPEG, "-y", "-f", "concat", "-safe", "0", "-i", voice_txt, "-c", "copy", voice_all])
    run(
        [
            FFMPEG,
            "-y",
            "-i",
            silent_video,
            "-i",
            voice_all,
            "-stream_loop",
            "-1",
            "-i",
            BGM,
            "-filter_complex",
            f"[1:a]volume=1.45[voice];[2:a]volume={BGM_VOLUME}[bgm];"
            "[voice][bgm]amix=inputs=2:duration=first:dropout_transition=1:normalize=0,"
            "alimiter=limit=0.95[a]",
            "-map",
            "0:v",
            "-map",
            "[a]",
            "-shortest",
            "-c:v",
            "copy",
            "-c:a",
            "aac",
            "-b:a",
            "192k",
            "-movflags",
            "+faststart",
            OUT,
        ]
    )
    shutil.copy2(OUT, FINAL_OUT)

    manifest = {
        "title": "CTって何分くらいかかるの？",
        "speaker": f"VOICEVOX speaker id {SPEAKER}",
        "voice_speed": VOICE_SPEED,
        "bgm": str(BGM),
        "bgm_volume": BGM_VOLUME,
        "frames": [str(path) for path in frames],
        "narration": NARRATION,
        "durations_seconds": [round(value, 3) for value in durations],
        "total_seconds": round(sum(durations), 3),
        "voice_audio": str(voice_all),
        "asset_video": str(OUT),
        "final_video": str(FINAL_OUT),
    }
    MANIFEST.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8-sig")
    print(FINAL_OUT)


if __name__ == "__main__":
    main()
