...
OUT = ASSET_DIR / "高濃度乳房って何.mp4"
FINAL_OUT = ROOT / "01_ショート動画_リール_YouTubeShorts" / "インスタ完成形" / "高濃度乳房って何.mp4"
...
TEXTS = [
    ...
    ["高濃度乳房は", "乳腺の割合が多い状態です"],
    ["病気ではなく", "乳房の特徴のひとつです"],
    ["マンモでは乳腺も", "病変も白く写ることがあります"],
    ["そのため病変が", "見えにくい場合があります"],
...
BG_FILES = [
    BG_DIR / "01_exam_room_density.png",
    BG_DIR / "01_exam_room_density.png",
    BG_DIR / "02_density_display.png",
    BG_DIR / "03_consultation.png",
    BG_DIR / "02_density_display.png",
    BG_DIR / "02_density_display.png",
    BG_DIR / "03_consultation.png",
    BG_DIR / "03_consultation.png",
...
def main():
    ...
    for idx, (lines, bg_file) in enumerate(zip(TEXTS, BG_FILES), start=1):
        ...
        frame.save(out)
        frames.append(out)
    cta_out = FRAME_DIR / "frame_11.png"
    ...
    for idx, text in enumerate(NARRATION, start=1):
        ...
        duration = max(3.0 if idx < 10 else 3.4, wav_duration(voice) + 0.45)
