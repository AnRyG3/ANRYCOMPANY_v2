...
- [[Characters/PATIENT_M02_001/PATIENT_M02_001]]
...
Codex側のPowerShell読み取りでBOMなしUTF-8の日本語ノートが崩れて見えることがあったため、よく使うObsidianノートと管理ノートをUTF-8 BOM付きに統一した。

対象:

- `ANRYCAMPANY/` 配下のMarkdownノート
- `00_全体管理/` 配下のMarkdownノート
- `00_全体管理/rules/` 配下のルールノート

今後、新しく日本語ノートを作る時もUTF-8 BOM付きで保存する。
