# DOCTOR_001

## 基本設定

| 項目 | 内容 |
| --- | --- |
| Character ID | DOCTOR_001 |
| 職業 | 医師 |
| 年齢 | 40〜45歳 |
| 性別 | 男性 |
| 体型 | 平均的な体型 |
| 髪型 | 短い黒髪。清潔感があり、整った専門職らしい髪型。 |
| 服装 | 白衣、ライトブルーのワイシャツ、ダークカラーのスラックス、病院IDバッジ |
| 役割 | 診断内容を説明し、必要な検査をすすめる。RT_TECH_001と連携して働く。 |

## 外見

- 日本人男性医師。
- 知的な印象。
- 温かく信頼できる笑顔。
- プロフェッショナルで自信がある。
- 健康的な印象。
- 患者さんに安心感を与える落ち着いた雰囲気。
- 実際の病院にいそうな自然な医師の見た目。

## 性格

- 落ち着いている。
- 知識が豊富。
- 専門的。
- 親切。
- 患者さんを安心させる。
- 説明が丁寧で、押しつけがましくない。

## 画像生成で守ること

- 実写に近い医療広告写真の質感にする。
- 清潔な病院、診察室、または検査説明の場面にする。
- 白衣、ライトブルーのワイシャツ、ダークカラーのスラックスを維持する。
- IDバッジには読める文字を入れない。
- 医師らしい落ち着きと信頼感を優先する。
- RT_TECH_001と並ぶ場合は、職種の違いが自然に伝わるようにする。
- 実在の人物や有名人に寄せない。

## 禁止事項

- 別人化。
- 派手な髪型。
- 金髪。
- アニメ化。
- 威圧的な表情。
- 不安をあおる表現。
- 読める病院名、ロゴ、氏名、資格証番号の表示。
- 美容広告のような過度な加工。

## 保存画像

| ファイル | 用途 |
| --- | --- |
| `reference_sheet.png` | 現在の基準画像。複数アングルと表情の参照用。 |
| `front.png` | 正面用。 |
| `left45.png` | 左45度用。 |
| `right45.png` | 右45度用。左45度カットを左右反転して作成。 |
| `side.png` | 横向き用。 |
| `expressions/warm_smile.png` | 温かい笑顔の表情参照。 |
| `expressions/` | 表情差分用フォルダ。 |

## 生成用プロンプト要約

Japanese male physician, age 40-45, photorealistic medical advertising photography, intelligent appearance, warm and trustworthy smile, professional and confident, healthy appearance, short black hair, neat and professional, average build, white coat, light blue dress shirt, dark trousers, hospital ID badge without readable text, calm, knowledgeable, kind, reassuring, explains diagnosis and recommends examinations, works together with RT_TECH_001, looks like a real hospital physician, fictional person, not based on any real person or celebrity.
